...
//TODO UserId와 StoreId로 리뷰를 검색하는 JPQL쿼리를 추가합니다.
    ...
    @Query("SELECT R FROM Review R WHERE R.user.id = :userId AND R.rating BETWEEN :startRating AND :endRating")
    Page<Review> findReviewByUserId(@Param("userId") Long userId,
                            @Param("startRating") int startRating,
                            @Param("endRating")  int endRating,
                            Pageable pageable);

    @Query("SELECT R FROM Review R WHERE R.store.id = :storeId AND R.rating BETWEEN :startRating AND :endRating")
